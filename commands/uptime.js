...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (!args2.toLowerCase().startsWith("http://"))
      return message.channel.send('URL must starts with "http://".');

    if (!args2.toLowerCase().endsWith(".glitch.me"))
      return message.channel.send('URL must end with ".glitch.me".');
    let database = db.get(`projects_${message.author.id}`);

    if (database && database.find(x => x.project === args2))
      return message.channel.send(":x: This is already Uptimed Project.");
